...
{  
  ...
  if (flex_return_code == LEX_CORRECT || flex_return_code == END_CHUNK) {
    //Possible return values are LEX_CORRECT and END_CHUNK: in both cases the content of the chunk, until its left cut point, has been stored into the string buffer.
    ...
    if(flex_return_code == END_CHUNK_ERROR || flex_return_code == END_OF_FILE)
    {
